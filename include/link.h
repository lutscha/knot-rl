...
struct Vertex {
  ...
  inline Vertex() : flags(0) {}

  Vertex(uint16_t under[2], uint16_t over[2], uint8_t flags) : flags(flags) {
...
private:
  ...
  inline uint16_t new_n_crossings(ReidemeisterKind kind) const {
    ...
    if (result < 0) {
      throw std::runtime_error("Negative n_crossings");
    };
  ...
  inline bool is_bigon(uint16_t a1) const noexcept {
    ...
    Visit &A1 = visits[a1];
    Visit &A2 = visits[a2];
    bool res = (A1.type() == VisitType::over && A2.type() == VisitType::over) && a1 != a2 && are_adjacent(mate(a1), mate(a2));
  ...
  inline bool is_triangle(uint16_t a_c, Direction dir_a, Direction dir_c) const noexcept {
    ...
    const uint8_t flag_cb = visits[c_b].crossing_flags(); // flag_a: C -> B
    const uint8_t flag_ab = visits[a_b].crossing_flags(); // flag_c: A -> B
  ...
  uint8_t compute_moves(uint16_t a) const noexcept {
    ...
    for (auto dir_a : {Direction::next, Direction::prev}) {
      for (auto dir_c : {Direction::next, Direction::prev}) {
        res |= is_triangle(a, dir_a, dir_c) << (Visit::DIR_SHIFT(dir_a, dir_c));
      }
    }
    return res;
  }

  void bind(uint16_t a, uint16_t b, uint8_t crossing_flags) {
  ...
  bool is_valid_move(uint16_t a, ReidemeisterMove move) const noexcept {
    if (a >= 2 * n_crossings) [[unlikely]] {
      if (n_crossings == 0) {
        if (move.kind != ReidemeisterKind::R1_pos) {  
          std::cerr << "Invalid move: only positive R1 is allowed on unknots, received " << move << std::endl;
          return false;
        }
        if (a != 0){
          std::cerr << "Invalid move: R1+ on an unknot should have index 0, received " << a << std::endl;
          return false;
        }
        return true;
      }
      std::cerr << "Invalid move: " << a << " is out of bounds for " << 2 * n_crossings << " crossings" << std::endl;
      return false;
    }
    ...
    case ReidemeisterKind::R3:
      return is_triangle(a, move.dir_over(), move.dir_under());
    default:
      return false;
    }
  }
  ...
  bool is_valid_R2(uint16_t a, const ReidemeisterMove &move) const noexcept {
    return true; // compute actual value
  }
...
public: // constructors
  STATIC
  Link(const uint16_t n_crossings, const uint16_t *n_conn_crossings, Visit *storage)
      : n_crossings(n_crossings), visits(storage) {
    std::memcpy(static_n_conn_crossings, n_conn_crossings,comp_cnt() * sizeof(uint16_t));
    compute_all_moves();
  }

  DYNAMIC Link (const uint16_t n_crossings, uint16_t components_cnt, uint16_t *storage, const uint16_t *n_conn_crossings)
      : n_crossings(n_crossings), 
       dynamic_components_cnt(components_cnt),
       dynamic_n_conn_crossings(storage),
 ...
 Link<static_n_components> apply_move(uint16_t v, ReidemeisterMove move, uint16_t *storage) const {
    ...
    if constexpr (dynamic) {
      return Link<static_n_components>(*this, a, move, storage);
    } else {
  ...
  STATIC Link(const Link<static_n_components> &link, uint16_t a, const ReidemeisterMove &move, Visit *storage)
      : n_crossings(link.new_n_crossings(move.kind)), visits(reinterpret_cast<Visit *>(storage)) {
    ...
    case ReidemeisterKind::R2_neg:
      ...
      break;
    case ReidemeisterKind::R2_pos:
      R2_pos(link, a, move.dir_over(), move.dir_under(), move.type());
      break;
    case ReidemeisterKind::R3:
  ...
  DYNAMIC Link(const Link<static_n_components> &link, uint16_t a,
      ...
      : n_crossings(link.new_n_crossings(move.kind)),
        ...
        dynamic_n_conn_crossings(storage),
        visits(reinterpret_cast<Visit *>(storage + link.dynamic_components_cnt)) {
  ...
  void R1_neg(const Link<static_n_components> &link,
    ...
    auto deloop = [a, a_next, &link](uint16_t i) {
      return Visit(delooping_index(a, a_next, link.visits[i].mate),
                   link.visits[i].flags);
    };

    uint16_t prev_ = delooping_index(a, a_next, link.prev(a));
    uint16_t next_ = delooping_index(a, a_next, link.next(a_next));
    ...
    if (!wrapping) {
      for (uint16_t i = 0; i < a; i++)
        visits[i] = deloop(i);
      for (uint16_t i = a + 2; i < 2 * link.n_crossings; i++)
        visits[i - 2] = deloop(i);
    } else {
      for (uint16_t i = 0; i < a_next; i++)
        visits[i] = deloop(i);
      for (uint16_t i = a_next + 1; i < a; i++)
        visits[i - 1] = deloop(i);
      for (uint16_t i = a + 1; i < 2 * link.n_crossings; i++)
        visits[i - 2] = deloop(i);
    }
  ...
  void R1_pos(const Link<static_n_components> &link, const uint16_t a, Orientation sign, VisitType type) noexcept { // loop (a, a+1)
  ...
  void R2_pos(const Link<static_n_components> &link, uint16_t a, uint16_t b, bool collinear, Orientation sign) noexcept { // a is over
    ...
    const uint8_t flag = Visit::FLAG(sign, VisitType::over);
  ...
  void R3(const Link<static_n_components> &link, const uint16_t a_c,
    ...
    uint8_t flag_b = visits[a_c].crossing_flags();
    uint16_t c_a = mate(a_c); // flag_b: A -> C
    uint16_t a_b = iter(a_c, dir_a);
    uint16_t c_b = iter(c_a, dir_c);
    uint8_t flag_a = visits[c_b].crossing_flags(); // flag_a: C -> B
    uint16_t b_c = mate(c_b);
    uint8_t flag_c = visits[a_b].crossing_flags(); // flag_c: A -> B
  ...
  std::pair<bool, std::string> verify_invariant() const noexcept {
    ...
    for (uint16_t a = 0; a < 2 * n_crossings; a++) {
      ...
      if (visits[a].type() == VisitType::under && visits[a].moves_flags() != 0) {
      ...
      if ((a + b) % 2 == 0){
        res << "Jordan's lemma violation: " << a << " and " << b << "are mates\n";
        violated = true;
      }
